...
import { parseUserQRData } from "@/lib/qr-code"
...
export function TransferForm() {
  ...
  const handleQRScan = (data: string) => {
    try {
      ...
      // Check if it's a user payment QR code
      if (parsedData.type === "user_payment") {
        const userQRData = parseUserQRData(data)

        if (userQRData) {
          // Set recipient to the user's ID
          setRecipient(userQRData.userId)
          setScannedMerchant({
            merchantId: userQRData.userId,
            merchantName: userQRData.name,
            type: "user_payment",
          })
          setIsQRScanned(true)

          toast({
            title: "User QR Code Scanned!",
            description: `Send money to ${userQRData.name}`,
          })
        }
      } else if (parsedData.type === "external_payment") {
        // External merchant QR code
  ...
  const handleTransfer = async (e: React.FormEvent) => {
    ...
    try {
      if (transferMethod === "qr" && scannedMerchant) {
        ...
        // Check if it's a user payment QR code or external merchant QR code
        if (scannedMerchant.type === "user_payment") {
          // User-to-user transfer via QR code
          const transferRequest = {
            FromUserId: user.UserId,
            ToUserId: scannedMerchant.merchantId,
            CurrencyCode: currency,
            Amount: parseFloat(amount),
          }

          console.log("[Transfer Form] Sending user QR transfer request:", transferRequest)
          ...
          try {
            ...
            addNotification(
              "transfer_sent",
              "Payment Sent",
              `Money sent to ${scannedMerchant.merchantName}`,
              amount,
              currency
            )
          ...
          console.log("[Transfer Form] Sending external payment request:", paymentPayload)
